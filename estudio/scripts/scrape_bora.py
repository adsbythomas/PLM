...
def main():
    ...
    # Guardar en src/data/ (el scraper vive en scripts/)
    import os
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "src", "data")
    os.makedirs(out_dir, exist_ok=True)

    with open(os.path.join(out_dir, "radar.json"), "w", encoding="utf-8") as f:
        json.dump({
            "updated": datetime.now().isoformat(),
            "items": derecho
        }, f, ensure_ascii=False, indent=2)

    with open(os.path.join(out_dir, "radar-contable.json"), "w", encoding="utf-8") as f:
        json.dump({
            "updated": datetime.now().isoformat(),
            "items": contable
        }, f, ensure_ascii=False, indent=2)

    print(f"src/data/radar.json: {len(derecho)} items")
    print(f"src/data/radar-contable.json: {len(contable)} items")
